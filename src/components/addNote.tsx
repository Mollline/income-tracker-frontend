...
export const AddNote: React.FC<AddNoteProps> = ({ notes, setNotes }) => {
    ...
    const handleCreateNote = async () => {
        ...
        if (!formData.noteTitle || !formData.note) {
    ...
    const handleOpen = () => {
        // Initialize noteId when opening modal
        ...
        setFormData({
            noteTitle: '',
            note: '',
            noteId: id || '' // Set noteId from localStorage or empty string
        });
        setOpen(true);
    };
                ...
                <div style={{ alignItems: 'center', display: 'flex', justifyContent: 'center', marginTop: '200px', }}>
                    <div className={styles.addRecord}>
                        <header className={styles.addRecordHeader}>
                            <div style={{ fontWeight: 'bold', color:'black' }}>Add note</div>
                        ...
                        <body className={styles.addRecordBody} style={{backgroundColor:"RGB(195,195,195)"}}>
                            <div >
                                <div style={{ display: 'flex', gap: '20px', margin: "20px", justifyContent: "center", alignItems: "center", }}>
                                    <div style={{ display: 'flex', gap: '5px', flexDirection: 'column', width: '396px', height: '444px' }}>
                                        <label style={{ color: '#1F2937' }}>Title:</label>
                                        <input
                                            style={{ width: "688px", height: '48px', border: '1px solid #D1D5DB', backgroundColor: '#F9FAFB', borderRadius: '5px', padding: '0px 50px',color:'black'}}
                                            maxLength={20}
                                            type="text"
                                            name="noteTitle"
                                        ...
                                        />
                                        <label style={{ color: '#1F2937' }}>Advice:</label>
                                        <textarea
                                            style={{ width: "688px", height: '150px', border: '1px solid #D1D5DB', backgroundColor: '#F9FAFB', borderRadius: '5px', padding: "15px 50px",color:'black' }}
                                            name="note"
                                            value={formData.note}
                                            onChange={handleInputChange}
                                        ></textarea>
