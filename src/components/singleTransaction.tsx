...
export const SingleTransaction: React.FC<Props>  = ({ transaction, transactions, setTransactions}) => {
    ...
    const handleDelete = async () => {
        ...
        console.log(id)
        try{
            const response = await axios.delete(`http://localhost:9999/deleteTransaction/${id}`)
    ...
    return (
        <div className={styles.selectOne} key={transaction._id}>
            ...
            <div style={{ display: 'flex', gap: '20px' }}>
                <div style={{ color: `${bgColor(transaction.transactionType)}` }}>{transaction.amount} MNT</div>
                <div>
                    <CiEdit />
